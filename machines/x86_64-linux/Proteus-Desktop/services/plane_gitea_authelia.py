...
class GiteaOAuthProvider(OauthAdapter):
    ...
    def __init__(self, request, code=None, state=None, callback=None):
        ...
        # Modified for Authelia
        auth_url = f"{GITEA_HOST}/api/oidc/authorization?{urlencode(url_params)}"
        ...
        super().set_user_data(
            {
                ...
                "user": {
                    # Modified for Authelia
                    "provider_id": str(user_info_response.get("id") or user_info_response.get("sub")),
                    "email": email,
                    # Modified for Authelia
                    "avatar": user_info_response.get("avatar_url") or "",
                    # Modified for Authelia
                    "first_name": user_info_response.get("name") or user_info_response.get("full_name") or user_info_response.get("preferred_username") or user_info_response.get("login") or "",
