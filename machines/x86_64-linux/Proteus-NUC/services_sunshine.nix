{
  ...
  in {
    allowedTCPPorts = [(lib.toInt s_https) (lib.toInt s_http) (lib.toInt s_rtsp)];
    allowedUDPPorts = [(lib.toInt s_video) (lib.toInt s_ctrl) (lib.toInt s_audio)];
  };
  ...
  services.sunshine = {
    ...
    settings = {
      adapter_name =
        if config.home-manager.users.${myvars.username}.wayland.windowManager.hyprland.nvidia_sync
