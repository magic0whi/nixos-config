{pkgs, ...}: let
  shared_cfg = {pkgs, ...}: {
    imports = [../modules/nixos_headless/bind.nix];
    ...
    services.bind = {
      ...
      domains."example.com" = {
        ...
        hosts = {
          ...
          # IPv4-only host
          Host-v4-only = [{ipv4 = "100.10.20.30";} {ipv4 = "10.0.0.4";}];
          # IPv6-only host
          Host-v6-only = [{ipv6 = "fd7a:115c:a1e0::1000";} {ipv6 = "fdfe:dcba:9877::4";}];
        };
      };
    };
  };
in
  pkgs.testers.runNixOSTest {
    ...
    nodes = {
      ...
      machine_mutable = {
        imports = [shared_cfg];
        services.bind.domains."example.com".mutable = true;
      };
    };
    ...
    testScript = ''
      ...
      run_dns_tests(machine_immutable)
      run_dns_tests(machine_mutable)
    '';
  }
